 ...
 *
 * Recent changes:
 * - 2026-05-15: Tightened HITL tool guidance so `ask_user_input` is used only after safe runtime inspection cannot resolve the missing information.
...
const BUILT_IN_TOOL_CATALOG: Record<BuiltInToolName, Omit<LLMToolDefinition, 'name' | 'execute'>> = {
  ...
  human_intervention_request: {
    description:
      'Legacy alias of `ask_user_input`. Prefer `ask_user_input` for new prompts. Use this tool only when the missing input cannot be safely resolved through read-only lookup, search, or inspection, or when approval, preference, or another human-only decision is required. Send questions[] with stable question and option ids. Supports single-select, multiple-select, and skip-capable prompts. Do not add a kind field; use type and allowSkip. Do not use allowSkip for approval-gated or otherwise blocking decisions; reserve it for explicitly dismissible, non-blocking prompts.',
