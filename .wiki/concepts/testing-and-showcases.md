...
- `tests/e2e/llm-turn-loop-hardening.ts` is deterministic: it uses scripted model responses plus real built-in tool execution to test recovery logic, repeated-tool recovery, timeout-after-tool-result diagnostics, hardening paths, and public cleanup semantics without live provider calls.
