...
- `tests/llm/turn-loop.test.ts` now covers hard-stop reasons, lifecycle hook ordering, synthetic tool-call marking, repeated-call suppression, timeout behavior, package-managed model dispatch, agent control-tool stops (`final_answer`, `need_user_input`, `blocked`), and the `complete(...)` default that rejects unresolved non-English and mixed-language text before any tool-result evidence exists.
- `tests/llm/runtime.test.ts` covers the runtime facade directly with temporary workspaces, including `createRuntime(...)`, `disposeRuntimeCaches()`, deterministic `search_files` results, idempotent `create_directory`, `path_exists`, and explicit rejection of the removed `grep` built-in name.
