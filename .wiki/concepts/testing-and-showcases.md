...
- `tests/llm/runtime.test.ts` covers the runtime facade directly with temporary workspaces, including `createRuntime(...)`, `streamComplete(...)`, text/reasoning/tool-call/final-answer delta events, control-tool termination, runtime forwarding of loop guardrails, deterministic `search_files` results, idempotent `create_directory`, `path_exists`, default-all built-ins, explicit built-in narrowing, the canonical `ask_user_input` schema, and explicit rejection of the removed `grep` built-in name.
