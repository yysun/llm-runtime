...
- `tests/llm/turn-loop.test.ts` now covers hard-stop reasons, lifecycle hook ordering, synthetic tool-call marking, repeated-call suppression, timeout behavior, package-managed model dispatch, and the `respondWithTools(...)` default that rejects unresolved non-English and mixed-language text before any tool-result evidence exists.
- `tests/llm/runtime.test.ts` covers the new filesystem built-ins directly with temporary workspaces, including deterministic `search_files` results, idempotent `create_directory`, `path_exists`, and explicit rejection of the removed `grep` built-in name.
...
- `tests/e2e/llm-turn-loop-azure-presentation.ts` and `tests/e2e/llm-turn-loop-gemini-presentation.ts` are presentation-oriented runners that force the model to load the `presentation` skill and read `README.md` plus `package.json` before drafting a five-slide storyboard.
...
This mix keeps the package regression-friendly while preserving both provider-specific live verification and realistic walkthrough paths for integration debugging and presentation generation. Related pages: [[src-turn-loop]], [[src-runtime]], [[src-mcp]], [[provider-adapters]], [[web-search-across-providers]], [[action-execution-hardening]], [[language-agnostic-continuation]], and [[turn-loop-safety-and-lifecycle]].
