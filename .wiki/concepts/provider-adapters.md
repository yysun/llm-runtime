...
- Function-calling adapters now also share `src/provider-tool-names.ts`, which sanitizes runtime tool names into provider-safe names, avoids reserved provider names, keeps long names bounded with deterministic hashes, and preserves reverse lookup so streamed and completed tool calls still resolve to the original runtime name.
- Streaming adapters emit deltas before the final `LLMResponse` is assembled. That matters for completion mode because the visible final answer may live inside the `final_answer` control tool's JSON arguments rather than ordinary assistant text.
