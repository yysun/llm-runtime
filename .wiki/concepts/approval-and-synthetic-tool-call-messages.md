...
- Default `ask_user_input` is model-visible but has no package executor. Here, HITL means "human in the loop": the product needs a real person to answer before it can continue safely.
- In runtime completion, default `ask_user_input` stops as `status: "tool_calls"` and returns the model's tool call to the host. Hosts answer by appending a normal tool result and resuming the loop.
- The runtime does not approve, wait, time out, fabricate a pending artifact, or resume the request on its own.
...
Read [[host-owned-ask-user-input]] for the ownership rule, [[src-builtins]] for the human-input payload shape, [[src-runtime-complete-contract]] for the public resume helper, and [[src-turn-loop]] for the compatibility path around synthetic tool calls.
