...
- Current built-in alias fixes include `path -> filePath` for `read_file` and `write_file`, `directory -> path` for `list_files`, `search_files`, and `create_directory`, `query -> pattern` for `search_files`, and `filePath -> path` for `path_exists`.
...
This page is central to the package's reliability model: malformed tool calls become explicit evidence that the host can persist and feed back into [[src-completion-loop]] instead of collapsing into ambiguous free-form text. The file-tool contract hardening tightened this path so missing `read_file` and `write_file` paths fail through normal validation rather than late executor fallback.
