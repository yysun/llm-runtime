...
- `ask_user_input` is now the preferred public human-input name, while `human_intervention_request` remains as a compatibility alias that uses the same structured schema.
- The human-input schema now requires `questions[]` with stable question ids and per-question option ids. It supports `single-select`, `multiple-select`, and optional `allowSkip` for explicitly dismissible prompts.
