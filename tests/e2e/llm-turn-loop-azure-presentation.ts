/**
 * Azure Presentation E2E Runner
 *
 * Purpose:
 * - Run the Azure-backed presentation workflow against the package turn loop.
 *
 * Key features:
...
async function runPresentationE2E(dryRun: boolean) {
  ...
  if (!selection && !dryRun) {
    console.error('No Azure provider configuration was found for the presentation turn-loop e2e.\n');
    console.error(getAzureE2EEnvHelp());
    process.exitCode = 1;
    return;
  }

  const azureSelection = selection ?? {
  ...
  const environment = createRuntime({
    providers: azureSelection.providers,
    skillRoots: [skillRoot],
  });

  try {
    console.log('LLM package Azure presentation turn-loop e2e');
    console.log(`provider=${azureSelection.provider}`);
    console.log(`model=${azureSelection.model}`);
    ...
    const result = await runCompletionLoop<PresentationState>({
        ...
        messages: [
          {
            role: 'system',
            content: [
              'You are running a strict Azure turn-loop e2e for llm-runtime.',
      ...
      callModel: async ({ messages }) => await generate({
        provider: azureSelection.provider,
        model: azureSelection.model,
        builtIns,
        messages,
        environment,
        context: {
...
async function main() {
  ...
  if (flags.help) {
    printProviderE2EHelp('test:e2e:azure:presentation', getAzureE2EEnvHelp());
    return;
  }
...
main().catch((error) => {
  console.error('azure presentation turn-loop e2e status: FAIL');
